  ...
  *
  ...
  class Pagination_Class
  {
    ...
    public function getResults()
    {
      ...
      if (empty($this->curPage) !== false && is_numeric($this->curPage) && substr(strval($this->curPage), 0, 1) != "-"  && $this->curPage != "0") {
      ...
      if (($this->pages) > 1) {
        ...
        if ($this->page != 1) {
          if ($this->_showFirstAndLast) {
			if($this->page - 1  != 1){
              $plinks[] = ' <a href="'.$queryUrl.'&amp;quiz_list=1"><span aria-hidden="true">&laquo;</span></a> ';
			}else{
			  $plinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&laquo;</span>';
			}
          }
          $plinks[] = ' <a href="'.$queryUrl.'&amp;quiz_list='.($this->page - 1).'"> &lsaquo; </a> ';
        } else {
		  if ($this->_showFirstAndLast) {
		    $plinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&laquo;</span>';
		  }
		  $plinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&lsaquo; </span>';
		}
        ...
        for ($j = 1; $j < ($this->pages + 1); $j++) {
          if ($this->page == $j) {
            ...
            $links[] = ' <a href="'.$queryUrl.'&amp;quiz_list='.$j.'">'.$j.'</a> '; // add the link to the array
          }
        }
        // Assign the 'next page' if we are not on the last page
        if ($this->page < $this->pages) {
          $slinks[] = ' <a href="'.$queryUrl.'&amp;quiz_list='.($this->page + 1).'"> &rsaquo; </a> ';
          if ($this->_showFirstAndLast) {
			if($this->page + 1 != $this->pages){
              $slinks[] = ' <a href="'.$queryUrl.'&amp;quiz_list='.($this->pages).'"> &raquo; </a> ';
			}else{
			  $slinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&raquo;</span>';
			}
          }
        }else{
		  $slinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&rsaquo;</span>';
          if ($this->_showFirstAndLast) {
            $slinks[] = '<span class="tablenav-pages-navspan" aria-hidden="true">&raquo;</span>';
          }
		}
        $input = '<span class="paging-input"><input class="current-page" id="current-page-selector" type="text" name="quiz_list" value="'.$this->page.'" size="1" aria-describedby="table-paging"> of <span class="total-pages">'.$this->pages.'</span></span>';
        // Push the array into a string using any some glue
        return implode(' ', $plinks).$input.implode(' ', $slinks);
      }
      return;
    }
  }